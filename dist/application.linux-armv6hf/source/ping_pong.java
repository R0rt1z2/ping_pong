...
/* Versió del joc */
float GAME_VERSION = 1.0f;

/* Colors */
int[] colors = {0xffFA0303, 0xffB305FF, 0xff00C647, 0xffF5BC00};
int colornum;
...
public void reset(int reset_booleans) {
...
public void draw() 
{
  if (in_menu != true && in_instructions != true) 
  {
    if (dead != true) 
    {
      ...
      {
        fill(colors[PApplet.parseInt(random(colors.length - 1))]);
      }
      ...
      {
        textSize(8);
        text("FPS: " + PApplet.parseInt(frameRate), width/2-50, height/2+330);
      }
...
public void draw_menu(int selection)
{
